function [RunsBid, RunsAsk] = RunFinalStage_v2(DATA, LinkBID, LinkASK)
            ...
            if isempty(index) && length(unique(RecordType))>1
                ...
                for k=2:length(LinkBID)
                    ...
                    if ID==id
                        ...
                        if ((Type(Start)==4 && Volume(Start)== size && Qualifiers(Start) == dir && ((time(Start)-time(END))<=100) && ((time(Start)-time(END))>=0)) || (Type(Start)==4 && Volume(Start)== size && Qualifiers(Start) == 2 && ((time(Start)-time(END))<=100) && ((time(Start)-time(END))>=0)))
                            % If the subsequent (k-th) trade message
                        ...
                        elseif (Type(Start)~=4 || Volume(Start)~= size || ((time(Start)-time(END))>100) || Qualifiers(Start) == 10000)
                            InferredLinkid=[];
                            InferredLinkID=[InferredLinkID;InferredLinkid];
                ...
                elseif ~isempty(index)
                    ...
                    for k=2:length(LinkBID)
                        ...
                        if ID==id
                            ...
                            if ((Type(Start)==4 && Volume(Start)== CancelVol && Qualifiers(Start) == dir && ((time(Start)-time(END))<=100) && ((time(Start)-time(END))>=0)) || (Type(Start)==4 && Volume(Start)== CancelVol && Qualifiers(Start) == 2 && ((time(Start)-time(END))<=100) && ((time(Start)-time(END))>=0)))    
                                % If the subsequent (k-th) trade message is
                                % (TRADE) of the same size and direction as
                                ...
                                InferredLink=[InferredLink;NextLink];
                                disp('linked messages found');
                            elseif (Type(Start)~=4 || Volume(Start)~= CancelVol || ((time(Start)-time(END))>100) || Qualifiers(Start) == 10000)
                                InferredLinkid=[];
                                InferredLinkID=[InferredLinkID;InferredLinkid];
                                InferredLink=[InferredLink;[]];
                                disp('linked messages not found');
